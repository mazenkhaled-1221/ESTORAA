...
/* =========================================================
   SUBSCRIBERS — LOCAL MIRROR
   ---------------------------------------------------------
   Every number that subscribes is also kept in localStorage.
   The admin dashboard reads Supabase first and falls back to
   this mirror, so the list still works while you are testing
   the site locally without the API.
========================================================= */

function mirrorSubscriber(phone) {
    ...
    try {
        ...
        console.warn(
            "ESTORA: Could not mirror the subscriber locally.",
            error
        );
...
newsletterForm?.addEventListener(
    ...
    async event => {
        ...
        try {
            ...
            /*
             * The API is unreachable (for example while the site
             * is opened directly from a folder). Save the number
             * locally so nothing is lost and the admin list still
             * shows it.
             */
...
/* =========================================================
   COMPANY WHATSAPP LINK (footer icon)
========================================================= */

const companyWhatsAppLink =
    document.getElementById("companyWhatsApp");
...
/* =========================================================
   CLICK THE PHOTO -> OPEN THE UNIT
   ---------------------------------------------------------
   The favourite button sits on top of the image, so it is
   excluded here.
========================================================= */

document.addEventListener("click", event => {

    if (event.target.closest(".property-favorite")) {
    ...
    const image =
        event.target.closest(
            ".property-image img, .property-image .property-no-image, .property-overlay"
        );
    ...
    const card =
        image.closest("[data-id], [data-property-id]");
...
/* =========================================================
   PULL THE LIVE UNITS FROM THE SERVER
   The page has already painted from the cache; this just
   refreshes it once the real list arrives.
========================================================= */
